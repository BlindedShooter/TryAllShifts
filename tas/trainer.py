...
class TASTrainer:
    def __init__(self, config: TASConfig):
        ...
        self.true_dynamics = avg_l1_norm(torch.ones(
            max(config.policy_rollout_len, config.imag_horizon), config.batch_size, config.dyn_enc_config.dyn_dim#, requires_grad=False
        ...
        # Losses
        self.wm_loss = calc_stoch_wm_loss if config.wm_config.dyn_model_config.stochastic else calc_wm_loss
    ...
    def train_wm(self, tran: Transition) -> InfoDict:
        ...
        wm_loss, wm_info = self.wm_loss(tran, self.wm, dynamics=self.true_dynamics[0])  # hmm
    ...
    def collect_experience(self, imag_horizon: Optional[int] = None) -> InfoDict:
        if imag_horizon is None:
            imag_horizon = self.config.policy_rollout_len
        ...
        imag_traj, info = self.wm.rollout_policy(
            tran.observations, self.actor.predict, rand_dynamics, horizon=imag_horizon,
            rew_pen=self.config.use_mopo_pen, lam=self.config.mopo_pen_lam
        )
        ...
        info['policy_eval/imag_rew_mean'] = imag_traj.rewards.mean().item()
    ...
    def learn_wm_and_dynenc(self) -> InfoDict:
        ...
        if self.config.use_dyn and self.train_states['wm_grad_updates'] % self.config.dynenc_update_freq == 0:
            traj = next(self.traj_gen).to_tensor(self.device)
            info.update(self.train_dynenc(traj))

        if self.config.wm_config.stochastic:
            info['obs_max_logvar'] = self.wm.obs_max_logvar.mean().item()
            info['obs_min_logvar'] = self.wm.obs_min_logvar.mean().item()
            info['reward_max_logvar'] = self.wm.reward_max_logvar.mean().item()
            info['reward_min_logvar'] = self.wm.reward_min_logvar.mean().item()
        return info
    ...
    def eval_wm(self) -> InfoDict:
        ...
        transfer_info = eval_wm_on_traj_oracledyn(self.wm, (self.dyn_encoder if self.config.use_dyn else None), exp_traj)
    ...
    def learn(self, wm_steps: int = 20000, sac_steps: int = 1000000):
        ...
        for _ in sac_pbar:
            ...
            if self.train_states['critic_grad_updates'] % self.config.policy_rollout_len == 0:
