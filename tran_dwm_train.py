...
for epoch in pbar:
    for np_tran in iter(dloader):
        info = {}
        dyn_latent.normal_()
        th_tran = np_tran.to_tensor(device=config.device)
        
        wm_loss, wm_info = world_model._calc_loss(th_tran)

        info.update(wm_info)

        dyn_latent.normal_()
        imag_traj = world_model.rollout(th_tran.observations, policy, dyn_latent, 24, detach=True)
        
        dyn_pred = dyn_encoder.trajectory_encode(imag_traj.detach())
